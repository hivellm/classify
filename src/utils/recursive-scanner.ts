...
export class RecursiveScanner {
   ...
   */
  private shouldSkipEntry(fullPath: string, relativePath: string, name: string): boolean {
    ...
    if (!this.includeTests) {
      const testPatterns = [/test/i, /spec/i, /__tests__/i, /__mocks__/i, /\.test\./, /\.spec\./];
  ...
  private categorizeFile(relativePath: string, filename: string): ScannedFile['category'] {
    ...
    // Entry points
    const entryFiles = ['index', 'main', 'app', '__init__', 'lib'];
